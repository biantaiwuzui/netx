package com.netx.admin.core.thread;
...
import com.netx.admin.core.util.MailUtil;
...
public class JobFailMonitorHelper {
	...
	private void failAlarm(XxlJobLog jobLog){
		...
		if (info!=null && info.getAlarmEmail()!=null && info.getAlarmEmail().trim().length()>0) {
			logger.info("发送报警邮件");
			String[] emailAddress = info.getAlarmEmail().split(",");
			for (String email: emailAddress) {
				logger.info("现发送邮件于："+email);
				...
				logger.info(MailUtil.sendMail(email, title, content)?"发送成功":"发送失败");
			}

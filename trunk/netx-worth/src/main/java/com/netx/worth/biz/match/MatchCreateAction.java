...
//        throw new RuntimeException("你没有权利做删除操作");
     ...
     * @return
     */
    public boolean updateMatchSatus(String matchId) {
...
//                    }
                ...
                break;
            case 2:
                map.put("matchMembers", matchServiceProvider.getMatchMemberService().IsWriteMatchMember(matchId));
    ...
    public boolean addOrUpdateMatchTicket(List<MatchTicketDTO> matchTicketDTOList, Date startTime, Date endTime) {
        ...
        for (MatchTicketDTO matchTicketDTO : matchTicketDTOList) {
            if (matchTicketDTO.isUseDefalut()) {
                MatchZone matchZone = matchServiceProvider.getMatchZoneService().selectById(matchTicketDTO.getMatchZoneOrMatchId());
                if (matchZone != null) {
                    MatchApplyDefaultTime matchApplyDefaultTime=matchServiceProvider.getMatchApplyDefaultTimeService().getMatchApplyDefaultTimeByMacthId(matchZone.getMatchId(), MatchTimeTypeEnum.MATCHTICKET.getType());
                    if(matchApplyDefaultTime!=null) {
                        matchTicketDTO.setBeginTime(matchApplyDefaultTime.getStartTime());
                        matchTicketDTO.setEndTime(matchApplyDefaultTime.getEndTime());
                    }
                }
            }

...
public class Money implements Serializable, Comparable<Object> {
     ...
     * @return
     */
    public static String getMoneyStringWithoutDecimal(long cent) {
        Money money = new Money();
        money.setCent(cent);
        String str = money.toString();
        String s[] = str.split("\\.");
        if(s.length >= 2){
            if("00".equals(s[1])) return s[0];
            else if(StringUtils.endsWith(s[1], "0")) return s[0] + "." + s[1].substring(0, 1);
            return Money.getMoneyString(cent);
        }
        return money.toString();
    }

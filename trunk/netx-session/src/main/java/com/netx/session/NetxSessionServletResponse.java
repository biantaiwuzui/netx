...
public class NetxSessionServletResponse extends HttpServletResponseWrapper {

	private static Log logger = LogFactory.getLog(NetxSessionServletResponse.class);
	 ...
	 */
	public NetxSessionServletResponse(HttpServletResponse response) {
		super(response);
	}

	public void addCookie(NetxCookie cookie) {
	...
	private String buildHttpOnlyCookie(NetxCookie cookie) {
	...
	public void flushBuffer() throws IOException {
		...
		if (writer != null) {
			if (logger.isDebugEnabled()) {
				logger.debug("com.netx.session framework flush writer");
			}
			writer.flush();
		}
		if (stream != null) {
			if (logger.isDebugEnabled()) {
				logger.debug("com.netx.session framework flush stream");
			}
			stream.flush();
		}
		this.flushed = true;
	...
	private void flushBufferAdapter() {
		if (streamAdapter != null) {
			if (logger.isDebugEnabled()) {
				logger.debug("com.netx.session framework flush streamAdapter");
			}
			streamAdapter.flush();
		}
		if (writerAdapter != null) {
			try {
				if (logger.isDebugEnabled()) {
					logger.debug("com.netx.session framework flush writerAdapter");
				}
				writerAdapter.flush();
			}
	...
	private String getCookieExpires(NetxCookie cookie) {
	...
	public void commitBuffer() throws IOException {
		if (status > 0) {
			if (logger.isDebugEnabled()) {
				logger.debug("com.netx.session framework commit : set status,status is : " + status);
			}
		...
		if (sendError != null) {
			if (sendError.message == null) {
				if (logger.isDebugEnabled()) {
					logger.debug("com.netx.session framework commit : set sendError,error status is : " + sendError.getStatus());
				}
				super.sendError(sendError.status);
			}
			else {
				if (logger.isDebugEnabled()) {
					logger.debug("com.netx.session framework commit : set sendError,error status is : " + sendError.getStatus() + " and the message is ： " + sendError.getMessage());
				}
			...
			if (logger.isDebugEnabled()) {
				logger.debug("com.netx.session framework commit : set sendRedirect,redirect url is  : " + sendRedirect);
			}
			super.sendRedirect(sendRedirect);
		}
		else if (stream != null) {
			if (logger.isDebugEnabled()) {
				logger.debug("com.netx.session framework commit : commit stream to the Servlet output stream");
			}
			flushBufferAdapter();
			...
			if (logger.isDebugEnabled()) {
				logger.debug("com.netx.session framework commit : commit writer to the Servlet output stream");
			}
			flushBufferAdapter();
		...
		if (this.flushed) {
			if (logger.isDebugEnabled()) {
				logger.debug("com.netx.session framework commit : call the servlet container flushBuffer");
			}
			super.flushBuffer();
		}
	}
	...
	public ServletOutputStream getOutputStream() throws IOException {
		...
		if (writer != null) {
			 ...
			 * 这样做会增加少量额外的内存开销，但标准的servlet engine不会遇到这种情形，只有少数 servlet
			 * engine需要这种做法（resin）
			 */
			if (writerAdapter != null) {
	...
	public PrintWriter getWriter() throws IOException {
		...
		if (this.isWriterBuffered) {
			if (logger.isDebugEnabled()) {
				logger.debug("ecommerceResponse.getWriter(): Created new character buffer");
			}
	...
	public void setSession(NetxSession session) {
	...
	/**
	 * 设置isWriterBuffered模式，如果设置成<code>true</code>
	 * ，表示将所有信息保存在内存中，否则直接输出到原始response中。 此方法必须在<code>getOutputStream</code>和
	 * <code>getWriter</code>方法之前执行，否则将抛出<code>IllegalStateException</code>。 *
	 * 
	 * @param isWriterBuffered
	 *            是否buffer内容
	 * @throws IllegalStateException
	 *             <code>getOutputStream</code>或<code>getWriter</code>方法已经被执行
	 */

...
import com.netx.session.util.BlowfishUtils;
import com.netx.session.NetxCookie;
...
public class CookieStore implements SessionStore {
    ...
    /**
     * undecodedCookies means the navtive cookies,which directly comes from
     * HttpRequest, the framework need to process it deeply.
     */
    ...
    private final NetxSessionServletResponse ecommerceResponse;
    ...
    public CookieStore(NetxSessionServletRequest ecommerceRequest, NetxSessionServletResponse ecommerceResponse) {
        this.ecommerceRequest = ecommerceRequest;
        this.ecommerceResponse = ecommerceResponse;
    }
    ...
    private Map<String, Attribute> decodeCombinedCookie(SessionConfigEntry sessionConfigEntry) {
        ...
        for (SessionConfigEntry configEntry : configEntries) {
            /**
             * if the cookie has been modified,the value will be the modified
             * value,don't need to decoded from cookies
             */
     ...
     *
     * @return the uncombined cookie map,the key is the single cookie name, the
     *         value is the single cookie value.
     */
    ...
    private Cookie[] getCookiesFromRequest() {
        if (ecommerceRequest == null)
            return new Cookie[0];
        return ecommerceRequest.getCookies();
    }
    ...
    public void commit() {
        ...
        /*
         * the combined cookie may be processed during the process of combined
		 * cookie
		 */
        for (String modifiedCookieName : tempCookieArray) {
            if (modifiedCookies.contains(modifiedCookieName)) {
                ...
                NetxCookie cookie = buildCookie(sessionConfigEntry);
                if (null == cookie) {
                    continue;
                }
                ecommerceResponse.addCookie(cookie);
            }
        }
        /**
    ...
    private NetxCookie buildCookie(SessionConfigEntry sessionConfigEntry) {
        if (sessionConfigEntry.isCommbine()) {
    ...
    private NetxCookie buildSingleCookie(SessionConfigEntry sessionConfigEntry) {
    ...
    private NetxCookie buildCookie(SessionConfigEntry sessionConfigEntry, boolean removed, String key, String value) {
        ...
        NetxCookie cookie = new NetxCookie(key, value);
        cookie.setHttpOnly(isHttpOnly);
        if (StringUtils.isNotBlank(sessionConfigEntry.getDomain())) {
            cookie.setDomain(sessionConfigEntry.getDomain());
        }
        cookie.setPath(StringUtils.isBlank(path) ? DEFAULT_PATH : path);
        cookie.setMaxAge(maxAge);
        return cookie;
    }

    private NetxCookie buildCombinedCookie(SessionConfigEntry sessionConfigEntry) {
    ...
    public SessionConfig getSessionConfig() {
        return ecommerceRequest.getSession().getSessionConfig();
    }
